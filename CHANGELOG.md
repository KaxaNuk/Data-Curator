...
- Five share count normalization calculations in the custom calculations template, so new projects get them and they are versioned and covered by tests: `c_normalized_weighted_average_basic_shares_outstanding`, `c_normalized_weighted_average_diluted_shares_outstanding`, and the `adjusted` basic, diluted and basic-and-diluted variants. Each restates a vendor's reported weighted average share count onto one current basis, since vendors restate for later corporate events inconsistently across period types, leaving the vendor's own columns untouched. The point-in-time `m_shares_outstanding` is the reference where the provider covers it; a period nothing explains is left null rather than guessed at
- `MarketDataDailyRow.shares_outstanding` (output column `m_shares_outstanding`): the point-in-time common shares outstanding, on the same split basis as the `*_split_adjusted` prices. Populated by the Intrinio provider from its daily `marketcap` series divided by the fully adjusted close; left unmapped, and so null, by the Financial Modeling Prep and LSEG providers

### Changed
- `c_market_cap` is now `m_close_split_adjusted * m_shares_outstanding` instead of `m_close_split_adjusted * fis_weighted_average_diluted_shares_outstanding`. The weighted average share counts are averages over a fiscal period rather than the count outstanding on a date, and Intrinio restates them for later splits on annual periods while leaving quarterly ones as-reported, which produced market cap jumps of whole split factors (up to 600% for AAPL). The market cap is now null before the provider's market cap coverage begins, rather than wrong

### Fixed
- Securities whose fundamentals dated several fiscal periods to a single filing are no longer rejected outright. A filing presents earlier periods as comparatives, and where a company never filed an original statement for those periods (Alcoa's 2016 separation dated fiscal 2014, 2015 and 2016 to one 2017-03-15 10-K) they collided on the fundamentals clock, which runs on the filing date. Only the latest period of each filing is now kept, with the omitted comparatives logged. This was rejecting well over half of a large universe with `Fundamental data unordered or duplicate dates`
- Periods whose income statement Intrinio serves as metadata only, carrying no line items, are now dropped as incomplete. They have no reporting currency either, since it is read off the line items' units, so they reached the output as an invalid currency or an empty non-nullable income statement and cost the security its whole fundamental data
- Market data dates whose every mapped field is null are now dropped instead of costing the security its market data. Intrinio serves empty rows for dates before a security started trading, and one such date in the first or last position crashed the run outright with an `AttributeError`
- Intrinio market capitalization values that are unusable are now dropped rather than reaching `m_shares_outstanding`: a literal `0.0` (GOOGL's 2007-2010), a stretch expressed in millions rather than units (Agilent's early 2010, stepping straight from `8565110166` to `9107`), and an isolated date left on a stale split basis (GOOGL's 2022-07-26, a twentieth of its neighbours a week after the 20-for-1 split)
- A security whose stock prices Intrinio serves an entirely empty numeric series for no longer loses its prices: every numeric price column is now explicitly typed instead of being inferred as null-typed and rejected as an all-null endpoint column. This covers the securities Intrinio serves no market capitalization for (so no derived shares outstanding) and the non-exchange-traded funds it reports no volume for, which were aborting the run with `Endpoints returned all-null columns`
- Intrinio HTTP 403 responses, returned for securities the subscription doesn't cover (delisted or OTC tickers), now raise `DataProviderAuthorizationError` and skip only that ticker, instead of aborting the entire run as a critical `ApiEndpointError`
